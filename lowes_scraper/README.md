...
## v191 path note
The web importer expects the exact normalized folder or ZIP path printed by the scrape command. A full run writes to paths like `./lowes_scraper/2026-export/normalized` and `./lowes_scraper/2026-normalized.zip`.

## v193 notes

- Direct summary probing now covers Lowe's All Orders, Online Orders, and Store Purchases tabs with `--order-types BOTH,ONLINE,INSTOREBACKROOM`.
- Rendered order-list rows now get stable direct detail URLs generated from the order number when Lowe's omits or misplaces the detail link in the captured DOM.
- Detail capture retries the stable direct URL if an encrypted/stale rendered link does not produce the expected order detail page.

## v194 Lowe's detail capture note

The scraper preserves Lowe's rendered detail URLs, including `s=` tokens, because some older online/pickup orders appear in summary pages but do not fully hydrate from the stable direct URL alone. Detail pages are retried several times before they are saved as `_unparsed`.

If an order remains in `lowes_detail_links_to_capture.csv`, open that detail URL manually in the same remote-debugging browser and use `save-current-cdp --parse` to save the loaded page.
