...
def diagnose_html_file(path: Path) -> Dict[str, Any]:
    ...
    reasons = []
    has_order_data = bool(summary_count or detail_order_number)
    ...
    if state and summary_count == 0 and not detail_order_number:
        reasons.append("state_decoded_but_no_order_history_or_order_details")
    if not reasons and (summary_count or detail_order_number):
        reasons.append("ok")
...
def normalize_payment(payment: Dict[str, Any], order_number: str, purchase_date: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "payment_key": safe_str(payment.get("paymentKey")),
...
def normalize_detail_state(state: Dict[str, Any], source_file: str = "") -> Optional[Dict[str, Any]]:
    ...
    payments = [normalize_payment(p, order_number, purchase_date) for p in data.get("orderPayments") or []]
...
def parse_html_file(path: Path) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    if (state.get("orderHistory") or {}).get("data"):
...
def collect_summary_pages_and_detail_links(page: Any, rawdir: Path, year: str, args: argparse.Namespace) -> List[str]:
    ...
    print(f"Collected {len(all_links)} candidate detail link(s) for {year} after pagination.")
    return all_links
            ...
            for i, link in enumerate(links, start=1):
                ...
                if state:
                    ...
                    if detail and detail.get("order_number"):
                        order_no = re.sub(r"[^0-9A-Za-z_.-]+", "_", detail["order_number"])
    ...
    with sync_playwright() as p:
        ...
        for year in years:
            ...
            for i, link in enumerate(links, start=1):
                ...
                page.goto(link, wait_until="domcontentloaded", timeout=60000)
                page.wait_for_timeout(args.settle_ms)
                # Extract order number from preloaded state where possible.
                content = page.content()
                order_no = f"{year}_{i:04d}"
                detail = None
                state = extract_preloaded_state(content)
