...
function run_amazon_invoice_repair_apply_script(string $gnucashPath, string $planPath, string $outPath, string $fromDate, string $toDate, bool $apply, string $logPath): array {
    ...
    if ($apply && !is_writable($gnucashPath)) return [2, 'Selected GnuCash book file is not writable by PHP-FPM. For web-run apply tests, make the apitest copy writable by the web user/group, or run the generated command manually as alan.'];
...
function import_vendor_file(SQLite3 $db, string $path, string $preferredVendor, string $gnucashPath=''): array {
    ...
    $fh = fopen($path, 'rb');
    if ($fh) { $hdr = fgetcsv($fh); fclose($fh); if ($hdr) { $fmt = detect_format(array_map('normalize_header_name', $hdr)); if ($fmt === 'walmart_orders') return import_walmart_csv($db, $path, $gnucashPath); if ($fmt === 'home_depot_purchase_history') return import_home_depot_csv($db, $path, $gnucashPath); } }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (($request['action'] ?? '') === 'import' && isset($_FILES['csv']) && is_uploaded_file($_FILES['csv']['tmp_name'])) {
        $chosenVendor = strtolower((string)($request['vendor'] ?? 'auto'));
        if (!in_array($chosenVendor, ['auto','amazon','costco','walmart','lowes','tractor_supply','home_depot'], true)) $chosenVendor = 'auto';
        $requestVendorHint = strtolower((string)($request['vendor_hint'] ?? ''));
        $requestVendorStep = strtolower((string)($request['vendor_step'] ?? ''));
        $uploadPrefix = @file_get_contents($_FILES['csv']['tmp_name'], false, null, 0, 4096);
        ...
        if ($chosenVendor === 'auto' && ($requestVendorHint === 'lowes' || ($uploadPrefix !== false && file_looks_like_zip($_FILES['csv']['tmp_name'], (string)$uploadPrefix)))) $chosenVendor = 'lowes';
        set_config($db, 'next_vendor_hint', $chosenVendor);
        [$count, $err, $itemCount] = array_pad(import_vendor_file($db, $_FILES['csv']['tmp_name'], $chosenVendor, (string)$gnucashPath), 3, null);
        if ($err) $error = $err; else {
            $message = vendor_import_postprocess_message($db, $chosenVendor, (int)$count, $itemCount, (string)$gnucashPath);
            if ($chosenVendor !== 'auto') $vendorHint = $chosenVendor;
            $mode = 'bills';
            if (in_array($vendorHint, ['lowes','home_depot','tractor_supply'], true)) $vendorStep = 'review';
            set_config($db, 'ui_mode', $mode);
    ...
    if (($request['action'] ?? '') === 'import_local_vendor_path') {
        ...
        if ($localPathInput === '') {
            ...
            if ($err) $error = $err; else {
                $message = vendor_import_postprocess_message($db, $chosenVendor, (int)$count, $itemCount, (string)$gnucashPath);
                $mode = 'bills'; set_config($db, 'ui_mode', $mode);
                $vendorHint = $chosenVendor; if (in_array($vendorHint, ['lowes','tractor_supply','home_depot'], true)) $vendorStep = 'review'; $page = 1; $filter = 'all'; $search = '';
            }
        }
    }
...
$lowesPaymentMappings = load_payment_account_map($db, 'lowes');
$lowesOutOfBookTargets = out_of_book_stage_targets($db, 'lowes');
...
<form method="post" id="addAccountForm" style="margin-bottom:1rem"><input type="hidden" name="action" value="add_account"><input type="hidden" name="gnucash_path" value="<?=h($gnucashPath)?>"><label>Add local account option:</label><input type="text" name="account_name" list="accounts" placeholder="Expenses:Farm Supplies:Feed"><button type="submit">Add account option</button> <span class="small">Adds to this tool's dropdown/autocomplete only; create the account in GnuCash proper before final import if it does not already exist.</span></form></details>
...
<form method="post" enctype="multipart/form-data"><input type="hidden" name="action" value="import"><input type="hidden" name="mode" value="bills"><input type="hidden" name="vendor_step" value="review"><input type="hidden" name="gnucash_path" value="<?=h($gnucashPath)?>"><label>Vendor/export type:</label><select name="vendor"><option value="auto">Auto-detect</option><option value="amazon">Amazon</option><option value="costco">Costco</option><option value="walmart">Walmart</option><option value="lowes">Lowe's</option><option value="home_depot">Home Depot</option><option value="tractor_supply">Tractor Supply</option></select><label>Vendor export file / normalized ZIP</label><input type="file" name="csv" accept=".csv,.json,.zip,text/csv,application/json,application/zip" required><button type="submit">Import / update vendor rows</button></form>
<p class="small">After import, continue to <a href="?mode=bills&vendor_hint=auto#review-top">Review Bills &amp; Line Items</a>. The importer keeps vendor context internally for account mappings, stored-value rows, and diagnostics.</p></div>
...
<div class="wizard-steps"><a class="wizard-step" href="#matching-lowes">Lowe's</a><a class="wizard-step" href="#matching-amazon">Amazon</a><a class="wizard-step" href="#matching-register-scans">Free-floating scans</a></div></div>
...
<div class="card" id="matching-amazon"><h3>Amazon matching</h3><p><a class="wizard-step" href="?mode=transactions&payment_wizard_step=start#payment-wizard">Open Amazon payment wizard</a></p></div>
...
<?php if($mode==='amazon'): ?>
<div class="wizard-steps"><a class="wizard-step <?=$vendorStep==='import'?'active':''?>" href="?mode=amazon&vendor_step=import#vendor-amazon">1. Import orders/items</a><a class="wizard-step" href="?mode=bills&vendor_hint=amazon#review-top">2. Categorize & export bills</a><a class="wizard-step" href="?mode=transactions&payment_wizard_step=start#payment-wizard">3. Import/payment mappings</a><a class="wizard-step" href="?mode=transactions&payment_wizard_step=apply#payment-wizard">4. Match/apply payments</a><a class="wizard-step" href="?mode=utility&utility_step=ledger#utility">5. Audit</a></div>
<div class="card" style="background:#f8fbff;border-color:#8bb7e0"><h3>Amazon active workflow</h3><ol class="small"><li>Upload Amazon order/item exports using the shared import control.</li><li>Review categories and export GnuCash bill CSVs.</li><li>Upload Amazon transaction-history CSVs from the payment wizard.</li><li>Run missing-invoice, exception, ready-plan, apply, and refund matching steps in order.</li></ol><p><a class="wizard-step" href="?mode=bills&vendor_hint=amazon#review-top">Open Amazon invoice/bill import and review</a> <a class="wizard-step" href="?mode=transactions&payment_wizard_step=start#payment-wizard">Open Amazon payment wizard</a></p></div>
...
CMD;
?>
<div class="card" id="vendor-tractor-supply-active"><h2>Tractor Supply workflow</h2><p class="small">Initial Tractor Supply module. It uses the same shared bill/category/export engine as Lowe's. Payment apply automation is not enabled yet, but free-floating register scans and payment mapping are available.</p><div class="wizard-steps"><a class="wizard-step <?=$vendorStep==='scrape'?'active':''?>" href="?mode=tractor_supply&vendor_step=scrape#vendor-tractor-supply-active">1. Scrape / parse saved pages</a><a class="wizard-step" href="?mode=bills&vendor_hint=tractor_supply#review-top">2. Categorize & export bills</a><a class="wizard-step <?=$vendorStep==='payments'?'active':''?>" href="?mode=tractor_supply&vendor_step=payments#vendor-unmatched-register-scan">3. Payment/register diagnostics</a><a class="wizard-step" href="?mode=ledger&ledger_vendor=tractor_supply#ledger-audit">4. Ledger audit</a></div></div>
...
<div class="wizard-steps lowes-workflow-nav"><a class="wizard-step <?=$vendorStep==='scrape'?'active':''?>" href="?mode=lowes&vendor_step=scrape#lowes-scraper">1. Scrape / normalize</a><a class="wizard-step <?=$vendorStep==='import'?'active':''?>" href="?mode=bills&vendor_hint=lowes&vendor_step=import#data-import-account-mapping">2. Data Import and Account Mapping</a><a class="wizard-step <?=$vendorStep==='review'?'active':''?>" href="?mode=bills&vendor_hint=lowes&vendor_step=review#review-top">3. Review Bills and Line Items</a><a class="wizard-step <?=$vendorStep==='stored_value'?'active':''?>" href="?mode=lowes&vendor_step=stored_value#vendor-lowes">4. Build &amp; export My Lowe's Money transactions</a><a class="wizard-step <?=$vendorStep==='match_dry_run'?'active':''?>" href="?mode=lowes&vendor_step=match_dry_run#vendor-lowes">5a. Transaction Matching Dry Run</a><a class="wizard-step <?=$vendorStep==='match_apply'?'active':''?>" href="?mode=lowes&vendor_step=match_apply#vendor-lowes">5b. Apply Transaction Matching to GnuCash File</a><a class="wizard-step <?=$vendorStep==='diagnostics'?'active':''?>" href="?mode=lowes&vendor_step=diagnostics#vendor-lowes">Diagnostics</a></div>
<?php render_lowes_step5_controls($db, 'lowes', (string)$vendorStep, 'lowes-payment-dry-run'); ?>
<?php if($vendorStep==='import'): ?>
<div class="card" style="background:#f8fbff;border-color:#8bb7e0"><h3>Step 2 — Data Import and Account Mapping</h3><p class="small">Import the Lowe's normalized ZIP/folder, then review the seeded payment methods and mapped accounts. This keeps dataset loading and payment-method setup separate from line-item categorization.</p><p><a class="wizard-step" href="?mode=bills&vendor_hint=lowes&vendor_step=import#data-import-account-mapping">Open Data Import and Account Mapping</a></p></div>
...
<div class="card" id="lowes-workflow-header"><h2>Lowe's workflow</h2><div class="wizard-steps lowes-workflow-nav"><a class="wizard-step" href="?mode=lowes&vendor_step=scrape#lowes-scraper">1. Scrape / normalize</a><a class="wizard-step <?=(($vendorStep==='import')?'active':'')?>" href="?mode=bills&vendor_hint=lowes&vendor_step=import#data-import-account-mapping">2. Data Import and Account Mapping</a><a class="wizard-step <?=(($vendorStep==='' || $vendorStep==='review')?'active':'')?>" href="?mode=bills&vendor_hint=lowes&vendor_step=review#review-top">3. Review Bills and Line Items</a><a class="wizard-step <?=(($vendorStep==='stored_value')?'active':'')?>" href="?mode=lowes&vendor_step=stored_value#vendor-lowes">4. Build &amp; export My Lowe's Money transactions</a><a class="wizard-step <?=(($vendorStep==='match_dry_run')?'active':'')?>" href="?mode=lowes&vendor_step=match_dry_run#vendor-lowes">5a. Transaction Matching Dry Run</a><a class="wizard-step <?=(($vendorStep==='match_apply')?'active':'')?>" href="?mode=lowes&vendor_step=match_apply#vendor-lowes">5b. Apply Transaction Matching to GnuCash File</a></div><p class="small">These workflow buttons remain available from the review page so you can move between Lowe's steps without returning to the vendor tab.</p><?php render_lowes_step5_controls($db, 'bills', (string)$vendorStep, 'data-import-account-mapping'); ?></div>
<?php endif; ?>
<details class="card collapsible-step" id="data-import-account-mapping" <?=count($allOrders)===0?'open':''?>><summary>2. Data Import and Account Mapping</summary>
<form method="post" enctype="multipart/form-data"><input type="hidden" name="action" value="import"><input type="hidden" name="mode" value="bills"><input type="hidden" name="vendor_hint" value="<?=h($vendorHint)?>"><input type="hidden" name="vendor_step" value="<?=h($vendorHint==='lowes' ? 'review' : $vendorStep)?>"><input type="hidden" name="gnucash_path" value="<?=h($gnucashPath)?>"><label>Vendor/export type:</label><select name="vendor"><option value="auto" <?=$vendorHint==='auto'?'selected':''?>>Auto-detect</option><option value="amazon" <?=$vendorHint==='amazon'?'selected':''?>>Amazon</option><option value="costco" <?=$vendorHint==='costco'?'selected':''?>>Costco</option><option value="walmart" <?=$vendorHint==='walmart'?'selected':''?>>Walmart</option><option value="lowes" <?=$vendorHint==='lowes'?'selected':''?>>Lowe's</option><option value="home_depot" <?=$vendorHint==='home_depot'?'selected':''?>>Home Depot</option><option value="tractor_supply" <?=$vendorHint==='tractor_supply'?'selected':''?>>Tractor Supply</option></select><label>Vendor export file: Amazon order/item CSV or JSON; Costco receipt JSON; Walmart order CSV; Lowe's/TSC normalized ZIP:</label><input type="file" name="csv" accept=".csv,.json,.zip,text/csv,application/json,application/zip" required><button type="submit">Import / update vendor rows</button>
</form>
<p class="small">For Lowe's scraper commands, ZIP creation, or direct normalized-folder import, use the <a href="?mode=lowes&vendor_step=scrape#lowes-scraper">Lowe's scraper</a> tab.</p>
</details>
<?php if($mode==='bills' && !empty($lowesPaymentMappings)): ?>
<div class="card" id="lowes-payment-mapping" style="background:#f8fbff;border-color:#8bb7e0;margin-top:.75rem"><h3>Lowe's payment method mapping / other-entity export</h3><p class="small">Lowe's normalized imports seed My Lowe's Money and credit-card methods from the payment rows. Credit-card accounts are guessed from the last four digits in GnuCash account names when possible. Mark another-entity card as <strong>Exclude invoices</strong>; those invoices/credits can then be exported as a separate bill CSV for the other entity book.</p>
<form method="post"><input type="hidden" name="action" value="save_payment_mappings"><input type="hidden" name="mode" value="bills"><table><thead><tr><th>Exclude</th><th>Payment method</th><th>Invoice handling</th><th>GnuCash account</th><th>External note/account</th></tr></thead><tbody><?php foreach($lowesPaymentMappings as $pm): $id=$pm['vendor'].'|'.$pm['method_key']; ?><tr><td><input type="checkbox" name="pm_excluded[]" value="<?=h($id)?>" <?=((int)($pm['excluded']??0)===1?'checked':'')?>></td><td><?=h($pm['display_name'])?><input type="hidden" name="pm_vendor[]" value="<?=h($pm['vendor'])?>"><input type="hidden" name="pm_key[]" value="<?=h($pm['method_key'])?>"><input type="hidden" name="pm_name[]" value="<?=h($pm['display_name'])?>"></td><td><select name="pm_handling[]"><option value="normal" <?=(($pm['invoice_handling']??'normal')==='normal'?'selected':'')?>>Normal / this book</option><option value="exclude_invoice" <?=(($pm['invoice_handling']??'')==='exclude_invoice'?'selected':'')?>>Exclude invoices / other entity</option><option value="external_expense" <?=(($pm['invoice_handling']??'')==='external_expense'?'selected':'')?>>Keep bill here, map payment to external clearing</option></select></td><td><input list="payment_accounts" name="pm_account[]" value="<?=h($pm['account_fullname'])?>" style="min-width:28rem"></td><td><input list="payment_accounts" name="pm_external_account[]" value="<?=h($pm['external_account'] ?? '')?>" placeholder="optional clearing/account note" style="min-width:22rem"></td></tr><?php endforeach; ?></tbody></table><button type="submit">Save Lowe's payment mappings</button></form>
<form method="post" style="margin-top:.75rem;display:flex;gap:.75rem;align-items:end;flex-wrap:wrap"><input type="hidden" name="action" value="export_out_of_book_bill_csv"><input type="hidden" name="mode" value="bills"><input type="hidden" name="out_of_book_vendor" value="lowes"><label><input type="checkbox" name="post_invoices" value="1" <?=$exportPostInvoicesChecked?'checked':''?>> Post invoices</label><label>Post date format<select name="post_date_format"><option value="mdy_slash" <?=$storedPostDateFormat==='mdy_slash'?'selected':''?>>MM/DD/YYYY</option><option value="dmy_slash" <?=$storedPostDateFormat==='dmy_slash'?'selected':''?>>DD/MM/YYYY</option><option value="iso" <?=$storedPostDateFormat==='iso'?'selected':''?>>YYYY-MM-DD</option></select></label><label>Posting/AP account<input list="accounts" type="text" name="posting_account" value="<?=h($storedPostingAccount)?>"></label><button type="submit">Export Lowe's other-entity bill CSV</button><span class="small"><?=count($lowesOutOfBookTargets)?> Lowe's invoice/credit document(s) currently match excluded payment methods. This export includes skipped rows so they can be imported into the other book.</span></form><?php if($lastPostAction==='export_out_of_book_bill_csv') echo render_action_report_html($message, $error, $exportLinks, 'Other-entity export report', 'export-report'); ?></div>
<?php endif; ?>
<?php endif; ?>
...
<div class="card" id="lowes-workflow-footer"><h2>Lowe's workflow</h2><div class="wizard-steps lowes-workflow-nav"><a class="wizard-step" href="?mode=lowes&vendor_step=scrape#lowes-scraper">1. Scrape / normalize</a><a class="wizard-step <?=(($vendorStep==='import')?'active':'')?>" href="?mode=bills&vendor_hint=lowes&vendor_step=import#data-import-account-mapping">2. Data Import and Account Mapping</a><a class="wizard-step <?=(($vendorStep==='' || $vendorStep==='review')?'active':'')?>" href="?mode=bills&vendor_hint=lowes&vendor_step=review#review-top">3. Review Bills and Line Items</a><a class="wizard-step <?=(($vendorStep==='stored_value')?'active':'')?>" href="?mode=lowes&vendor_step=stored_value#vendor-lowes">4. Build &amp; export My Lowe's Money transactions</a><a class="wizard-step <?=(($vendorStep==='match_dry_run')?'active':'')?>" href="?mode=lowes&vendor_step=match_dry_run#vendor-lowes">5a. Transaction Matching Dry Run</a><a class="wizard-step <?=(($vendorStep==='match_apply')?'active':'')?>" href="?mode=lowes&vendor_step=match_apply#vendor-lowes">5b. Apply Transaction Matching to GnuCash File</a></div><p class="small">Footer copy for long review pages: use this after saving mappings, validating, or exporting without scrolling back to the top.</p><?php render_lowes_step5_controls($db, 'bills', (string)$vendorStep, 'lowes-workflow-footer'); ?></div>
<?php endif; ?>
</div>
<?php endif; ?>
<script>
