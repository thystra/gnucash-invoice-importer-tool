    ...
    if (($request['action'] ?? '') === 'import_local_vendor_path') {
        ...
        if ($localPathInput === '') {
            ...
            if ($err) $error = $err; else {
                ...
                $mode = 'import_data'; set_config($db, 'ui_mode', $mode);
...
<div class="card" id="data-import-account-mapping"><h3>2. Data Import and Account Mapping</h3><p class="small"><strong>Import additional files on step 3 tab.</strong> The Review Bills tab is for category/account review and bill export. Amazon orders, items, and transaction-history CSVs should be imported from the Import Data tab before returning here.</p><p><a class="wizard-step" href="?mode=import_data#import-shared">Open Step 3 — Import Data</a></p></div>
<?php else: ?>
